...
def data_from_web():
    ...
    dataset_select_label.grid()
    tk.OptionMenu(import_screen, dataset_var, *dataset_names).pack()
    select_button = tk.Button(import_screen, text="Select", command=import_data2(dataset_var)).grid()

    label = tk.Label(import_screen)
    label.grid()
    import_data2(dataset_var)
    #import_screen.mainloop()


def import_data2(dataset_var):
    global data
